...
switch(msgid)
{
    ...
    case 1: // INFO_CMD
        ...
        show_debug_message(newDifficulty)
        show_debug_message(global.oldDifficulty)

        if (global.oldDifficulty != newDifficulty) {
            scr_set_difficulty(newDifficulty);
            show_debug_message("Changed difficulty");
        }
    ...
    case 7: // BASKETBALL
        ...
        var ballX = buffer_read(buffer, buffer_s32);
        var ballY = buffer_read(buffer, buffer_s32);
        var ballH = buffer_read(buffer, buffer_f32);
        var ballV = buffer_read(buffer, buffer_f32);

        var ball = instance_create_layer(ballX, ballY, "Player", obj_ball);

        ball.hspeed = ballH;
        ball.vspeed = ballV; */

        // Unused code above was generated by github copilot might be useful later
        break;
